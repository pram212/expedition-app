@extends('layouts.app')
@section('title', 'Entry Konsumen')
@section('header', 'Entry Konsumen')
...
@section('content')
    <div class="card">
        <div class="card-header bg-secondary">
            Entry Konsumen Via Admin
        </div>
        <div class="card-body">
            @php
                $url = (@$order) ? '/crm/order/' . @$order->id : '/crm/order/' ;
            @endphp
            <form action="{{ url($url) }}" method="POST">
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id"
                                ...
                                @foreach ($categories as $item)
                                    <option value="{{ $item->id }}" @if ( $item->id == old('category_id', @$order->category_id ) )
                                        selected
                    ...
                    <div class="col-sm-12 col-md-6">
                        <div class="form-group">
                            ...
                            <select name="payment_statuses_id"
                                class="form-control select2 @error('payment_statuses_id') is-invalid @enderror"
                                id="payment_statuses_id">
                                <option value="">-- Pilih --</option>
                                @foreach ($paymentStatus as $item)
                                    <option value="{{ $item->id }}" @if ( $item->id == old('payment_statuses_id', @$order->payment_statuses_id ) )
                                        selected
                                    @endif>{{ strtoupper($item->name) }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-sm-12 col-md-6">
                        <div class="form-group">
                            <label for="shippment_statuses_id">Status Pengiriman : <span
                                    class="text-danger">*</span></label>
                            <select name="shippment_statuses_id"
                                class="form-control select2 @error('shippment_statuses_id') is-invalid @enderror"
                                id="shippment_statuses_id">
                                <option value="">-- Pilih --</option>
                                @foreach ($shipmentStatus as $item)
                                    <option value="{{ $item->id }}" @if ( $item->id == old('shippment_statuses_id', @$order->shippment_statuses_id ) )
                                        selected
                                    @endif>
                    ...
                    <div class="col-sm-12 col-md-6">
                        <div class="form-group">
                            ...
                            <select name="district_id"
                                class="form-control select2 @error('district_id') is-invalid @enderror" id="district_id">
                                <option value="">-- Pilih --</option>
                                @foreach ($districts as $item)
                                    <option value="{{ $item->id }}" 
                                        @if ( $item->id == old('district_id', @$order->district_id ) )
                                            selected
                                        @endif
                                    >
                                        {{ $item->name }}
                    ...
                    <div class="col-sm-12 col-md-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('address') is-invalid @enderror"
                                placeholder="maksimal 65 karakter & jangan sertakan kelurahan / kecamatan" maxlength="65" name="address"
                                id="address" rows="3">{{ old('address', @$order->address) }}</textarea>
